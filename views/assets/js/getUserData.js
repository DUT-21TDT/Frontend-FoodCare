...
$(document).ready(async function () {
    ...
    $('.menu_page-btn').pagination({
        ...
        afterPageOnClick: function (event, pageNumber) {
            loadMenu(menuUserData.data.list, pageNumber);
        },
        beforeInit: function (event, pageNumber) {
            loadMenu(menuUserData.data.list, 1);
        }
    })
...
$(`.menu_search-input`).change(async () => {
    ...
    const filteredData = menuUserData.data.list.filter(food => food.menuname.toLowerCase().includes(searchText.toLowerCase()) || food.creator.toLowerCase().includes(searchText.toLowerCase()));
